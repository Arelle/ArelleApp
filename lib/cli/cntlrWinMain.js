...
var rlAccessionEvent;
var rlAccessionViews = {};
...
function connectToDB() {
    var win = rlPopupWindows.createWindow({
        id:"connectToDB",
        x:20, y:30, width:260, height:198, center:true
...
function openAccession() {
    var accessionId = null, win, layout, grid, form;
    ...
    win.setText("Open an accession");
    grid = win.attachGrid();
    grid.setHeader("Accession,Timestamp,URL,Creation Software,Messages");
    grid.setInitWidths("100,60,100,200,400");
    ...
    grid.load("/grid/accessions", "json");
    rlStatusbar.setText("Select an accession to view...");
}

function closeAccession() {
	if (rlCurrentTabOrdinal in rlAccessionViews) {
		var accessionView = rlAccessionViews[rlCurrentTabOrdinal];
		var tabItems = accessionView['tabItems'];
		for (var i=0; i<tabItems.length; i++) {
			var tabItem = tabItems[i];
			tabItem['tabBar'].removeTab(tabItem['tabName'],true);
			// tabItem['grid'].destructor();
		}
		delete rlAccessionViews[rlCurrentTabOrdinal];
	}
}

function onSelectObject(accessionView, id, clickedTabName) {
	try {
		var tabItems = accessionView['tabItems'];
		for (var i=0; i<tabItems.length; i++) {
			var tabItem = tabItems[i];
			if (clickedTabName != tabItem['tabName'] && tabItem['showObjectFunction']) {
				tabItem['showObjectFunction'](tabItem['grid'], accessionView['accessionId'], id, tabItem['arcrole']);
			}
		}
	} catch (err) {
...
function propertyGridShowObject(grid, accessionId, id) {
    grid.clearAll();
    grid.load("/grid/properties?accessionId=" + accessionId + "&id=" + id, 'json');
}

function treeGridShowObject(grid, accessionId, id, arcrole) {
...
function aspectsGridShowObject(grid, accessionId, id) {
...
function dataPointsGridShowObject(grid, accessionId, id) {
    urlApiResultsFunction("Selecting data point", "/select/datapoints?id=" + id, function(results){
        grid.selectRowById(results[0], false, true, false);
    });
}

function newGridItem(name, accessionView, tabBar, width, colHeaders, colWidths, colAligns, multiline, colTypes, colSorts, arcrole, loadUrl, setInitialViewActive, onRowSelectFunction, showObjectFunction) {
	var tabNameSuffix = '';
	var tabLabelSuffix = '';
	var tabWidth = width;
	if (rlOpenOrdinal > 1) {
    ...
    if (onRowSelectFunction) {
	    grid.attachEvent("onRowSelect", function(id,ind){ // onRowSelect, onRowDblClicked
	    	setTimeout(function(){
  						onRowSelectFunction(accessionView, id, tabName);
  						}, 300);
    	});
    }
    var gridObj = {};
    ...
    gridObj['accessionId'] = accessionView['accessionId'];
    if (loadUrl) {
    	loadUrl += "?accessionId=" + accessionView['accessionId'];
    	if (arcrole)
    ...
    accessionView['tabItems'].push(gridObj);
}

function viewAccession(accessionId) {
	var accessionView = {};
	accessionView['accessionId'] = accessionId;
	var tabItems = new Array();
	accessionView['tabItems'] = tabItems;
	rlOpenOrdinal += 1;
	rlCurrentTabOrdinal = rlOpenOrdinal.toString();
	rlAccessionViews[rlCurrentTabOrdinal] = accessionView;
	newGridItem("Messages", accessionView, rlTabbarBottom, 60,
				"Seq,Code,Level,Text",
				"32,180,120,600",
	...
	newGridItem("Fact List", accessionView, rlTabbarTopRight, 60,
	...
	newGridItem("Aspects", accessionView, rlTabbarBottom, 60,
	...
	newGridItem("Presentation", accessionView, rlTabbarTopRight, 72,
	...
	newGridItem("Calculation", accessionView, rlTabbarTopRight, 68,
				"Name,Weight,Balance",
				"400,50,70",
				...
				"/grid/relationships", false, onSelectObject, treeGridShowObject);
	newGridItem("Dimensions", accessionView, rlTabbarTopRight, 72,
	...
	newGridItem("DTS", accessionView, rlTabbarTopLeft, 32,
				"Document",
				"400",
				"left", false,
				"tree",
	...
	newGridItem("Properties", accessionView, rlTabbarTopLeft, 72,
				"Property,Value",
				"100,200",
				"left,left", false,
				"tree,ro",
