...
function openFiling(onSelect, param1) {
    ...
    if (args && args.length >= 3) {
        ...
        selectFilingsUrl = cgi + "/grid/filings?filing=" + args[0] + "&name=" + args[1] + "&date=" + args[2] + "&sic=" + args[3];
    } else {
        args = new Array("", "", "", "");
    ...
    winToolbar.addText("_filing", 1, "Filing");
    winToolbar.addInput("filing", 2, args[0], 132);
    winToolbar.addText("_name", 3, "Name");
    winToolbar.addInput("name", 4, args[1], 300);
    winToolbar.addText("_date", 5, "Date");
    winToolbar.addInput("date", 6, args[2], 64);
    winToolbar.addText("_sic", 7, "SIC");
    winToolbar.addInput("sic", 8, args[3], 36);
    ...
    winToolbar.attachEvent("onClick", function(id){
        if (id == "find") {
        	args = new Array(winToolbar.getValue("filing"),  winToolbar.getValue("name"), winToolbar.getValue("date"), winToolbar.getValue("sic"));
	        createCookie("rlFilingSelection", Base64.encode(args.join('\t')), 365 * 5);
	        grid.clearAll();
		    grid.load(cgi + "/grid/filings?filing=" + args[0] + 
		    	      "&name=" + args[1] + "&date=" + args[2] + "&sic=" + args[3],
		              "json");
        } else
            alert("Toolbar button with name "+id+" was clicked");
    });
    grid = win.attachGrid();
    grid.setHeader("Filing,Name,Timestamp,URL,SIC,Creation Software,Messages");
    grid.setInitWidths("132,184,124,48,36,64,400");
    	...
    	setTimeout(function(){ 
    				 if (onSelect == "openFiling")
    				     viewObject('filing', id)
    				 	 ...
    				 	 openMultivariateGrid(id);
    				 else if (onSelect == "addToMultivariateGrid")
    				 	 addToMultivariateGrid(id, param1);
    			  }, 200);
        return true;
    });
...
function openMultivariateGrid(filingId) {
	viewMultivariate(filingId);
}
...
function viewMultivariate(filingId) {
	var objectView = {
		filingIds: [filingId],
	...
	newGridItem("Filings", objectView, rlTabbarBottom, 60,
				"Filing,Name,Timestamp,URL,SIC,Creation Software",
				"132,184,124,48,36,64",
				"left,left,left,left,left,left", false,
				"ro,ro,ro,ro,ro,ro",
				"str,str,str,str,str,str",
				"menuVaribleRules",
...
function addToMultivariateGrid(filingId, objectView) {
	objectView.filingIds.push(filingId);
	refreshMultivariateLoadUris(objectView);
}
