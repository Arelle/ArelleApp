...
function viewMultivariate(filingIds) {
	...
	var mvGridColTypes = mvGrid.getColType(0), mvGridColAligns = "left";
	for (var iCol=0; iCol<objectView.variables.length; iCol+=1) {
		...
		mvGridColTypes += ",ron"; // numeric col
		mvGridColAligns += ",right";
	}
	mvGrid.setColTypes(mvGridColTypes);
	mvGrid.setColAlign(mvGridColAligns);
	// load grids
    openTabItemGrids(objectView.tabItems);
}
